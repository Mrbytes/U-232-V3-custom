...
if (($users_friends = $mc1->get_value($keys['user_friends'])) === false) {
    $fr = sql_query("SELECT f.friendid as uid, f.userid AS userid, u.last_access, u.id, u.ip, u.avatar, u.username, u.class, u.donor, u.title, u.warned, u.enabled, u.chatpost, u.leechwarn, u.pirate, u.king, u.downloaded, u.uploaded, u.perms FROM ".TBL_FRIENDS." AS f LEFT JOIN ".TBL_USERS." as u ON f.friendid = u.id WHERE userid=".sqlesc($id)." ORDER BY username ASC LIMIT 100") or  sqlerr(__file__, __line__);
...
if (count($users_friends) > 0) {
    $user_friends = "<table width='100%' class='main' border='1' cellspacing='0' cellpadding='5'>\n"."<tr><td class='colhead' width='20'>Avatar</td><td class='colhead'>Username".($CURUSER['class'] >= UC_STAFF ? "/Ip" : "")."</td><td class='colhead' align='center'>Uploaded</td>".($INSTALLER09['ratio_free'] ? "" : "<td class='colhead' align='center'>Downloaded</td>")."<td class='colhead' align='center'>Ratio</td><td class='colhead' align='center'>Status</td></tr>\n";
    if ($users_friends) {
        foreach ($users_friends as $a) {
            $avatar = ($user['avatars'] == 'yes' ? ($a['avatar'] == '' ? '<img src="'.$INSTALLER09['pic_base_url'].'default_avatar.gif"  width="40" alt="default avatar" />' : '<img src="'.htmlsafechars($a['avatar']).'" alt="avatar"  width="40" />') : '');
        ...
        if (empty($users_friends)) $HTMLOUT.= "<tr><td colspan='2'>No Friends yet.</td></tr>";
    }
}
//== thee end
//==end
