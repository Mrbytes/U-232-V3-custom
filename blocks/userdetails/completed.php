...
if ($user['paranoia'] < 2 || $CURUSER['id'] == $id || $CURUSER['class'] >= UC_POWER_USER) {
    ...
    $r = sql_query("SELECT ".TBL_TORRENTS.".name, ".TBL_TORRENTS.".added AS torrent_added, ".TBL_SNATCHED.".start_date AS s, ".TBL_SNATCHED.".complete_date AS c, ".TBL_SNATCHED.".downspeed, ".TBL_SNATCHED.".seedtime, ".TBL_SNATCHED.".seeder, ".TBL_SNATCHED.".torrentid as tid, ".TBL_SNATCHED.".id, ".TBL_CATEGORIES.".id as category, ".TBL_CATEGORIES.".image, ".TBL_CATEGORIES.".name as catname, ".TBL_SNATCHED.".uploaded, ".TBL_SNATCHED.".downloaded, ".TBL_SNATCHED.".hit_and_run, ".TBL_SNATCHED.".mark_of_cain, ".TBL_SNATCHED.".complete_date, ".TBL_SNATCHED.".last_action, ".TBL_TORRENTS.".seeders, ".TBL_TORRENTS.".leechers, ".TBL_TORRENTS.".owner, ".TBL_SNATCHED.".start_date AS st, ".TBL_SNATCHED.".start_date FROM ".TBL_SNATCHED." JOIN ".TBL_TORRENTS." ON ".TBL_TORRENTS.".id = ".TBL_SNATCHED.".torrentid JOIN  ".TBL_CATEGORIES." ON  ".TBL_CATEGORIES.".id = ".TBL_TORRENTS.".category WHERE ".TBL_SNATCHED.".finished='yes' AND userid=".sqlesc($id)." AND ".TBL_TORRENTS.".owner != ".sqlesc($id)." ORDER BY ".TBL_SNATCHED.".id DESC") or sqlerr(__FILE__, __LINE__);
    //=== completed
    ...
    <td class='colhead' align='center'>{$lang['userdetails_speed']}</td></tr>";
        while ($a = mysqli_fetch_assoc($r)) {
            ...
            else $dl_speed = mksize(($a["downloaded"] / ($a['c'] - $a['s'] + 1)));
            switch (true) {
            case ($dl_speed > 600):
                $dlc = 'red';
                break;
    ...
    if ($completed && $CURUSER['class'] >= UC_POWER_USER || $completed && $user['id'] == $CURUSER['id']) {
        if (!isset($_GET['completed'])) $HTMLOUT.= tr('<b>'.$lang['userdetails_completedt'].'</b><br />', '[ <a href=\'./userdetails.php?id='.$id.'&amp;completed=1#completed\' class=\'sublink\'>Show</a> ]&nbsp;&nbsp;-&nbsp;'.mysqli_num_rows($r) , 1);
        elseif (mysqli_num_rows($r) == 0) $HTMLOUT.= tr('<b>'.$lang['userdetails_completedt'].'</b><br />', '[ <a href=\'./userdetails.php?id='.$id.'&amp;completed=1\' class=\'sublink\'>Show</a> ]&nbsp;&nbsp;-&nbsp;'.mysqli_num_rows($r) , 1);
        else $HTMLOUT.= tr('<a name=\'completed\'><b>'.$lang['userdetails_completedt'].'</b></a><br />[ <a href=\'./userdetails.php?id='.$id.'#history\' class=\'sublink\'>Hide list</a> ]', $completed, 1);
    }
}
//==End hnr
// End Class
