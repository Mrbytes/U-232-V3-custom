...
if (($topics = $mc1->get_value('last_posts_' . $CURUSER['class'])) === false) {
    $topicres = sql_query("SELECT t.id, t.user_id, t.topic_name, t.locked, t.forum_id, t.last_post, t.sticky, t.views, t.anonymous AS tan, f.min_class_read, f.name " . ", (SELECT COUNT(id) FROM ".TBL_POSTS." WHERE topic_id=t.id) AS p_count " . ", p.user_id AS puser_id, p.added, p.anonymous AS pan " . ", u.id AS uid, u.username " . ", u2.username AS u2_username " . "FROM ".TBL_TOPICS." AS t " . "LEFT JOIN ".TBL_FORUMS." AS f ON f.id = t.forum_id " . "LEFT JOIN ".TBL_POSTS." AS p ON p.id=(SELECT MAX(id) FROM  ".TBL_POSTS." WHERE topic_id = t.id) " . "LEFT JOIN ".TBL_USERS." AS u ON u.id=p.user_id " . "LEFT JOIN ".TBL_USERS." AS u2 ON u2.id=t.user_id " . "WHERE f.min_class_read <= " . $CURUSER['class'] . " " . "ORDER BY t.last_post DESC LIMIT {$INSTALLER09['latest_posts_limit']}") or sqlerr(__FILE__, __LINE__);
...
if (count($topics) > 0) {
    ...
    if ($topics) {
        foreach ($topics as $topicarr) {
            ...
            if ($topicarr['pan'] == 'yes') {
                if ($CURUSER['class'] < UC_STAFF && $topicarr['user_id'] != $CURUSER['id']) $username = (!empty($topicarr['username']) ? "<i>Anonymous</i>" : "<i>Unknown</i>");
                else $username = (!empty($topicarr['username']) ? "<i>Anonymous</i><br /><a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . (int)$topicarr['puser_id'] . "'><b>[" . htmlsafechars($topicarr['username']) . "]</b></a>" : "<i>Unknown[$topic_userid]</i>");
            } else {
                $username = (!empty($topicarr['username']) ? "<a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . (int)$topicarr['puser_id'] . "'><b>" . htmlsafechars($topicarr['username']) . "</b></a>" : "<i>Unknown[$topic_userid]</i>");
            }
            if ($topicarr['tan'] == 'yes') {
                if ($CURUSER['class'] < UC_STAFF && $topicarr['user_id'] != $CURUSER['id']) $author = (!empty($topicarr['u2_username']) ? "<i>Anonymous</i>" : ($topic_userid == '0' ? "<i>System</i>" : "<i>Unknown</i>"));
                else $author = (!empty($topicarr['u2_username']) ? "<i>Anonymous</i><br /><a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . $topic_userid . "'><b>[" . htmlsafechars($topicarr['u2_username']) . "]</b></a>" : ($topic_userid == '0' ? "<i>System</i>" : "<i>Unknown[$topic_userid]</i>"));
            } else {
                $author = (!empty($topicarr['u2_username']) ? "<a href='" . $INSTALLER09['baseurl'] . "/userdetails.php?id=" . $topic_userid . "'><b>" . htmlsafechars($topicarr['u2_username']) . "</b></a>" : ($topic_userid == '0' ? "<i>System</i>" : "<i>Unknown[$topic_userid]</i>"));
            }
