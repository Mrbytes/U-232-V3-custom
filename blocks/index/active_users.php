...
if (($active_users_cache = $mc1->get_value($keys['activeusers'])) === false) {
    ...
    $res = sql_query('SELECT id, username, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king '.'FROM '.TBL_USERS.' WHERE last_access >= '.$dt.' AND perms < '.bt_options::PERMS_STEALTH.' ORDER BY username ASC') or sqlerr(__FILE__, __LINE__);
