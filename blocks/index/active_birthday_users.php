...
if (($birthday_users_cache = $mc1->get_value($keys['birthdayusers'])) === false) {
    ...
    $res = sql_query("SELECT id, username, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king, birthday FROM ".TBL_USERS." WHERE MONTH(birthday) = ".sqlesc($current_date['mon'])." AND DAYOFMONTH(birthday) = ".sqlesc($current_date['mday'])." AND perms < ".bt_options::PERMS_STEALTH." ORDER BY username ASC") or sqlerr(__FILE__, __LINE__);
...
if (!$birthday_users_cache['birthdayusers']) $birthday_users_cache['birthdayusers'] = 'There is no members birthdays today.';
