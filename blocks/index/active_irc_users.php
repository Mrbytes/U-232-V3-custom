...
if (($active_irc_users_cache = $mc1->get_value($keys['activeircusers'])) === false) {
    ...
    $res = sql_query('SELECT id, username, irctotal, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king '.'FROM '.TBL_USERS.' WHERE onirc = "yes" '.'AND perms < '.bt_options::PERMS_STEALTH.' ORDER BY username ASC') or sqlerr(__FILE__, __LINE__);
...
if (!$active_irc_users_cache['activeircusers']) $active_irc_users_cache['activeircusers'] = 'There have been no active irc users in the last 15 minutes.';
