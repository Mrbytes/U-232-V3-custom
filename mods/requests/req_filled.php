...
$res = sql_query("SELECT id FROM ".TBL_TORRENTS." WHERE id = $torrentid") or sqlerr(__FILE__, __LINE__);
$arr = mysqli_fetch_assoc($res);
if (!$arr) stderr("{$lang['error_error']}", "{$lang['error_no_torrent2']}$torrentid");
$res = sql_query("SELECT ".TBL_USERS.".username, ".TBL_REQUESTS.".userid, ".TBL_REQUESTS.".torrentid, ".TBL_REQUESTS.".request FROM ".TBL_REQUESTS." INNER JOIN ".TBL_USERS." ON ".TBL_REQUESTS.".userid = ".TBL_USERS.".id WHERE ".TBL_REQUESTS.".id = $id") or sqlerr(__FILE__, __LINE__);
$arr = mysqli_fetch_assoc($res);
...
sql_query("UPDATE ".TBL_REQUESTS." SET torrentid = ".$torrentid.", filledby = $CURUSER[id] WHERE id = $id") or sqlerr(__FILE__, __LINE__);
sql_query("INSERT INTO ".TBL_MESSAGES." (poster, sender, receiver, added, msg, subject, location) VALUES(0, 0, $arr[userid], ".TIME_NOW.", ".sqlesc($msg).", 'Request Filled', 1)") or sqlerr(__FILE__, __LINE__);
//$Cache->delete_value('inbox_new_'.$arr['userid'].'');
if ($INSTALLER09['karma'] && isset($CURUSER['seedbonus'])) sql_query("UPDATE ".TBL_USERS." SET seedbonus = seedbonus+".$INSTALLER09['req_comment_bonus']." WHERE id = $CURUSER[id]") or sqlerr(__FILE__, __LINE__);
$res = sql_query("SELECT `userid` FROM ".TBL_REQUEST_VOTES." WHERE `requestid` = $id AND userid != $arr[userid]") or sqlerr(__FILE__, __LINE__);
$msgs_buffer = array();
