 ...
 *
 */
$res = sql_query('SELECT * FROM '.TBL_REQUEST_VOTES.' WHERE requestid = '.$id.' and userid = '.$CURUSER['id']) or sqlerr(__FILE__, __LINE__);
...
<br /><br />";
    ...
    sql_query('UPDATE '.TBL_REQUESTS.' SET hits = hits+1 WHERE id='.$id) or sqlerr(__FILE__, __LINE__);
    if (mysqli_affected_rows($GLOBALS["___mysqli_ston"])) {
        sql_query('INSERT INTO '.TBL_REQUEST_VOTES.' VALUES(0, '.$id.', '.$CURUSER['id'].')') or sqlerr(__FILE__, __LINE__);
        $HTMLOUT.= "
<h3>Vote accepted</h3>
