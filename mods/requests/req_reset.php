...
$res = sql_query("SELECT userid, filledby, request, torrentid FROM ".TBL_REQUESTS." WHERE id = $id") or sqlerr(__FILE__, __LINE__);
$arr = mysqli_fetch_assoc($res);
if (($CURUSER['id'] == $arr['userid']) || ($CURUSER['class'] >= UC_MODERATOR) || ($CURUSER['id'] == $arr['filledby'])) {
    if ($INSTALLER09['karma'] && isset($CURUSER['seedbonus']) && $arr['torrentid'] != 0) sql_query("UPDATE ".TBL_USERS." SET seedbonus = seedbonus-".$INSTALLER09['req_comment_bonus']." WHERE id = $arr[filledby]") or sqlerr(__FILE__, __LINE__);
    sql_query("UPDATE ".TBL_REQUESTS." SET torrentid = 0, filledby = 0 WHERE id = $id") or sqlerr(__FILE__, __LINE__);
