 ...
 *
 */
$res = sql_query('SELECT userid, offer FROM '.TBL_OFFERS.' WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
...
if (!isset($_GET['sure'])) stderr('Delete Offer', "You`re about to delete this offer. Click\n <a class='altlink' href='viewoffers.php?id=$id&amp;del_offer&amp;sure=1'>here</a>, if you`re sure.", false);
    ...
    sql_query('DELETE FROM '.TBL_OFFERS.' WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
    sql_query('DELETE FROM voted_'.TBL_OFFERS.' WHERE offerid = '.$id) or sqlerr(__FILE__, __LINE__);
    sql_query('DELETE FROM '.TBL_COMMENTS.' WHERE offer = '.$id) or sqlerr(__FILE__, __LINE__);
