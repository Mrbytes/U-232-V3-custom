...
if ($CURUSER['class'] >= UC_MODERATOR) {
    ...
    sql_query("DELETE FROM ".TBL_OFFERS." WHERE id IN (".implode(", ", array_map("sqlesc", $_POST['deloff'])).")");
    sql_query("DELETE FROM voted_offers WHERE offerid IN (".implode(", ", array_map("sqlesc", $_POST['deloff'])).")");
    sql_query("DELETE FROM ".TBL_COMMENTS." WHERE offer IN (".implode(", ", array_map("sqlesc", $_POST['deloff'])).")");
