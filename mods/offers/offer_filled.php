...
$res = sql_query("SELECT ".TBL_USERS.".username, ".TBL_OFFERS.".userid, ".TBL_OFFERS.".torrentid, ".TBL_OFFERS.".offer FROM ".TBL_OFFERS." inner join ".TBL_USERS." on ".TBL_OFFERS.".userid = ".TBL_USERS.".id where ".TBL_OFFERS.".id = $id") or sqlerr(__FILE__, __LINE__);
$arr = mysqli_fetch_assoc($res);
...
sql_query("UPDATE ".TBL_OFFERS." SET torrentid = ".$torrentid.", acceptedby = $CURUSER[id] WHERE id = $id") or sqlerr(__FILE__, __LINE__);
...
if ($INSTALLER09['karma'] && isset($CURUSER['seedbonus'])) sql_query("UPDATE ".TBL_USERS." SET seedbonus = seedbonus+".$INSTALLER09['offer_comment_bonus']." WHERE id = $CURUSER[id]") or sqlerr(__FILE__, __LINE__);
