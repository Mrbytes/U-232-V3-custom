...
$res = sql_query("SELECT userid, acceptedby, offer, torrentid FROM ".TBL_OFFERS." WHERE id = $id") or sqlerr(__FILE__, __LINE__);
$arr = mysqli_fetch_assoc($res);
if (($CURUSER['id'] == $arr['userid']) || ($CURUSER['class'] >= UC_MODERATOR) || ($CURUSER['id'] == $arr['acceptedby'])) {
    if ($INSTALLER09['karma'] && isset($CURUSER['seedbonus']) && $arr['torrentid'] != 0) sql_query("UPDATE ".TBL_USERS." SET seedbonus = seedbonus-".$INSTALLER09['offer_comment_bonus']." WHERE id = $arr[acceptedby]") or sqlerr(__FILE__, __LINE__);
