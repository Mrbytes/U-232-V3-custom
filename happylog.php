...
$ur = sql_query("SELECT username from ".TBL_USERS." WHERE id=".sqlesc($id));
$user = mysqli_fetch_array($ur) or stderr("Error", "No user found");
$count = get_row_count(TBL_HAPPYLOG, "WHERE userid=".sqlesc($id));
$perpage = 30;
$pager = pager($perpage, $count, "happylog.php?id=$id&amp;");
$res = sql_query("SELECT h.userid, h.torrentid, h.date, h.multi, t.name FROM ".TBL_HAPPYLOG." as h LEFT JOIN ".TBL_TORRENTS." AS t on t.id=h.torrentid WHERE h.userid=".sqlesc($id)." ORDER BY h.date DESC ".$pager['limit']) or sqlerr(__FILE__, __LINE__);
