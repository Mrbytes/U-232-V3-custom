...
for ($i = 97; $i < 123; ++$i) {
    $l = chr($i);
    $L = chr($i - 32);
    if ($l == $letter) $htmlout.= "<font class=\"sublink-active\">$L</font>\n";
    else $htmlout.= "<a class=\"sublink\" href=\"".$_SERVER["PHP_SELF"]."?letter=".$l."\">".$L."</a>\n";
}
...
} else $htmlout.= "<h2>Nothing found!</h2>";
$htmlout.= end_frame();
echo stdhead('Catalogue').$htmlout.stdfoot();
?>
