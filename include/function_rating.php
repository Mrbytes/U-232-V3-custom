...
function getRate($id, $what)
{
    ...
    if (($rating_cache = $mc1->get_value($keys['rating'])) === false) {
        $qy = sql_query("SELECT sum(r.rating) as sum, count(r.rating) as count, r2.id as rated, r2.rating  FROM ".TBL_RATING." as r LEFT JOIN ".TBL_RATING." as r2 ON (r2.".$what." = ".sqlesc($id)." AND r2.user = ".sqlesc($CURUSER["id"]).") WHERE r.".$what." = ".sqlesc($id)." GROUP BY r.".$what) or sqlerr(__FILE__, __LINE__);
