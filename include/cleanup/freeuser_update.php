...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, modcomment FROM ".TBL_USERS." WHERE free_switch > 1 AND free_switch < ".TIME_NOW) or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERS." (id, free_switch, modcomment) VALUES ".implode(', ', $users_buffer)." ON DUPLICATE key UPDATE free_switch=values(free_switch), modcomment=concat(values(modcomment),modcomment)") or sqlerr(__FILE__, __LINE__);
