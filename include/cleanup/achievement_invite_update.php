...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, invited, inviterach FROM ".TBL_USERSACHIEV." WHERE invited >= '1'") or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            if ($invited >= 1 && $lvl == 0) {
                $msg = sqlesc("Congratulations, you have just earned the [b]Inviter Level 1[/b] achievement. :) [img]http://u-232.com/pic/achievements/invite1.png[/img]");
            ...
            if ($invited >= 2 && $lvl == 1) {
                $msg = sqlesc("Congratulations, you have just earned the [b]Inviter Level 2[/b] achievement. :) [img]http://u-232.com/pic/achievements/invite2.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 3 && $lvl == 2) {
                $msg = sqlesc("Congratulations, you have just earned the [b]Inviter Level 3[/b] achievement. :) [img]http://u-232.com/pic/achievements/invite3.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 5 && $lvl == 3) {
                $msg = sqlesc("Congratulations, you have just earned the [b]Inviter Level 4[/b] achievement. :) [img]http://u-232.com/pic/achievements/invite4.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($invited >= 10 && $lvl == 4) {
                $msg = sqlesc("Congratulations, you have just earned the [b]Inviter Level 5[/b] achievement. :) [img]http://u-232.com/pic/achievements/invite5.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
