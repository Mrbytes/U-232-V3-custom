...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, dailyshouts, dailyshoutlvl FROM ".TBL_USERSACHIEV." WHERE dailyshouts >= '10'") or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            if ($shouts >= 1 && $lvl == 0) {
                $msg = sqlesc("Congratulations, you have just earned the [b]ShoutBox Spammer Level 1[/b] achievement. :) [img]http://u-232.com/pic/achievements/spam1.png[/img]");
            ...
            if ($shouts >= 25 && $lvl == 1) {
                $msg = sqlesc("Congratulations, you have just earned the [b]ShoutBox Spammer Level 2[/b] achievement. :) [img]http://u-232.com/pic/achievements/spam2.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 50 && $lvl == 2) {
                $msg = sqlesc("Congratulations, you have just earned the [b]ShoutBox Spammer Level 3[/b] achievement. :) [img]http://u-232.com/pic/achievements/spam3.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 75 && $lvl == 3) {
                $msg = sqlesc("Congratulations, you have just earned the [b]ShoutBox Spammer Level 4[/b] achievement. :) [img]http://u-232.com/pic/achievements/spam4.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
            ...
            if ($shouts >= 100 && $lvl == 4) {
                $msg = sqlesc("Congratulations, you have just earned the [b]ShoutBox Spammer Level 5[/b] achievement. :) [img]http://u-232.com/pic/achievements/spam5.png[/img]");
                $msgs_buffer[] = '(0,' . $arr['id'] . ',' . TIME_NOW . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ')';
