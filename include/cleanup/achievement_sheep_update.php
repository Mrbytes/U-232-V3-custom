...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, sheepyset FROM ".TBL_USERSACHIEV." WHERE sheepyset = '1' AND sheepyach = '0'") or sqlerr(__FILE__, __LINE__);
    $msg_buffer = $usersachiev_buffer = $achievements_buffer = array();
    if (mysqli_num_rows($res) > 0) {
        $subject = sqlesc("New Achievement Earned!");
        $msg = sqlesc("Congratulations, you have just earned the [b]Sheep Fondler[/b] achievement. :) [img]http://u-232.com/pic/achievements/sheepfondler.png[/img]");
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERSACHIEV." (id, sheepach, achpoints) VALUES " . implode(', ', $usersachiev_buffer) . " ON DUPLICATE key UPDATE sheepach=values(sheepach), achpoints=achpoints+values(achpoints)") or sqlerr(__FILE__, __LINE__);
