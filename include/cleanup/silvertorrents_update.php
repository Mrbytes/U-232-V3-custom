...
function docleanup($data)
{
    ...
    ignore_user_abort(1);
    //=== Clean silver
    $res = sql_query("SELECT id, silver FROM ".TBL_TORRENTS." WHERE silver > 1 AND silver < ".TIME_NOW) or sqlerr(__FILE__, __LINE__);
    $Silver_buffer = array();
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($count > 0) {
            sql_query("INSERT INTO ".TBL_TORRENTS." (id, silver) VALUES ".implode(', ', $Silver_buffer)." ON DUPLICATE key UPDATE silver=values(silver)") or sqlerr(__FILE__, __LINE__);
