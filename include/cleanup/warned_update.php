...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, modcomment FROM ".TBL_USERS." WHERE warned > 1 AND warned < ".TIME_NOW) or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERS." (id, warned, modcomment) VALUES ".implode(', ', $users_buffer)." ON DUPLICATE key UPDATE warned=values(warned), modcomment=concat(values(modcomment),modcomment)") or sqlerr(__FILE__, __LINE__);
