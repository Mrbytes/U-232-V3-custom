...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT ".TBL_USERS.".id, ".TBL_USERS.".corrupt, ".TBL_USERSACHIEV.".corrupt FROM ".TBL_USERS." LEFT JOIN ".TBL_USERSACHIEV." ON ".TBL_USERS.".id = ".TBL_USERSACHIEV.".id WHERE enabled = 'yes' AND ".TBL_USERS.".corrupt >= '1' AND usersachiev.corrupt = '0'") or sqlerr(__FILE__, __LINE__);
    $msg_buffer = $usersachiev_buffer = $achievements_buffer = array();
    if (mysqli_num_rows($res) > 0) {
        $subject = sqlesc("New Achievement Earned!");
        $msg = sqlesc("Congratulations, you have just earned the [b]Corruption Counts[/b] achievement. :) [img]http://u-232.com/pic/achievements/corrupt.png[/img]");
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERSACHIEV." (id, corrupt, achpoints) VALUES " . implode(', ', $usersachiev_buffer) . " ON DUPLICATE key UPDATE corrupt=values(corrupt), achpoints=achpoints+values(achpoints)") or sqlerr(__FILE__, __LINE__);
