...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, modcomment FROM ".TBL_USERS." WHERE avatarpos > 1 AND avatarpos < ".TIME_NOW) or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERS." (id, avatarpos, modcomment) VALUES ".implode(', ', $users_buffer)." ON DUPLICATE key UPDATE avatarpos=values(avatarpos), modcomment=concat(values(modcomment),modcomment)") or sqlerr(__FILE__, __LINE__);
