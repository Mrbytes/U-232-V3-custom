...
function docleanup($data)
{
    ...
    $res = sql_query("SELECT id, modcomment FROM ".TBL_USERS." WHERE game_access > 1 AND game_access < ".TIME_NOW) or sqlerr(__FILE__, __LINE__);
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO ".TBL_USERS." (id, game_access, modcomment) VALUES ".implode(', ', $users_buffer)." ON DUPLICATE key UPDATE game_access=values(game_access), modcomment=concat(values(modcomment),modcomment)") or sqlerr(__FILE__, __LINE__);
