...
function get_reputation($user, $mode = '', $rep_is_on = TRUE)
{
    ...
    if ($rep_is_on) {
        ...
        if ($mode != '') return "Rep: ".$posneg."<br /><br /><a href='javascript:;' onclick=\"PopUp('{$INSTALLER09['baseurl']}/reputation.php?pid={$user['id']}&amp;locale=".$mode."','Reputation',400,241,1,1);\"><img src='{$INSTALLER09['pic_base_url']}forumicons/giverep.jpg' border='0' alt='Add reputation:: {$user['username']}' title='Add reputation:: {$user['username']}' /></a>";
        else return " ".$posneg;
    } // END IF ONLINE
    // default
