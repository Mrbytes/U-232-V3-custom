...
function invincible($id, $invincible = true, $bypass_bans = true)
{
    ...
    // grab current data
    $res = sql_query('SELECT username, passkey, ip, perms, modcomment FROM '.TBL_USERS.' 
                     WHERE id = '.sqlesc($id).' LIMIT 1') or sqlerr(__file__, __line__);
    $row = mysqli_fetch_assoc($res);
    $row['perms'] = (int)$row['perms'];
    // delete from iplog current ip
    sql_query('DELETE FROM '.TBL_IPS.' WHERE userid = '.sqlesc($id)) or sqlerr(__file__, __line__);
    ...
    //ipf = '.sqlesc($ip).',
    sql_query('UPDATE '.TBL_USERS.' SET ip = '.sqlesc($ip).', modcomment = '.sqlesc($modcomment).'
