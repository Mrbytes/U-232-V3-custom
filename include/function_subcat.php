...
function categories_table($cats, $wherecatina, $linkpage = '', $display = 'block')
{
 ...
 <img border=\"0\" src=\"pic/plus.png\" id=\"pic".(int)$cat['tabletype']."\" alt=\"Show/Hide\" />&nbsp;".htmlsafechars($cat['name'])."</a>&nbsp;".(($linkpage != '') ? "<a class=\"catlink\" href=\"{$linkpage}?{$cat['categories']}\">(All)</a>" : "")."</td>\n";
        $i++;
    }
    $nrows = ceil($ncats / $INSTALLER09['catsperrow']);
    $lastrowcols = $ncats % $INSTALLER09['catsperrow'];
    if ($lastrowcols != 0) {
        if ($INSTALLER09['catsperrow'] - $lastrowcols != 1) $html.= "<td class=\"one\" rowspan=\"".($INSTALLER09['catsperrow'] - $lastrowcols)."\">&nbsp;</td>";
        else $html.= "<td class=\"one\">&nbsp;</td>";
    }
...
function subcategories_table($cats, $wherecatina = array() , $linkpage = '', $ncats)
{
    ...
    $i = 0;
    if (count($subcats) > 0) foreach ($subcats as $cat) {
 ...
 ".($CURUSER['browse_icons'] == 'yes' ? "<img src='{$INSTALLER09['pic_base_url']}caticons/{$CURUSER['categorie_icon']}/".htmlsafechars($cat['image'])."' alt='".htmlsafechars($cat['name'])."' title='".htmlsafechars($cat['name'])."' />" : "".htmlsafechars($cat['name'])."")."</a>" : htmlsafechars($cat['name']))."</td>\n";
    ...
    if ($lastrowcols != 0) {
        if ($INSTALLER09['subcatperrow'] - $lastrowcols != 1) $html.= "<td class=\"one\" rowspan=\"".($INSTALLER09['subcatperrow'] - $lastrowcols)."\">&nbsp;</td>";
        else $html.= "<td class=\"one\">&nbsp;</td>";
    }
    $html.= "</tr></tbody></table></div>";
    return $html;
    ...
    foreach ($cats as $cat) {
        ...
        if (count($subcats) > 0) {
            foreach ($subcats as $subcat) {
                if ($subcat['id'] == $subcatid) return True;
            }
        }
    }
    return False;
