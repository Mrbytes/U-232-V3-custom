...
case 'add':
    ...
    if ($sure === false) {
        ...
        if (mysqli_num_rows($q1) === 1) {
            ...
            if (isset($errs[$err_key])) stderr('Err', sprintf($errs[$err_key], $ar1['username']));
            else stderr('Confirm action', sprintf('%s you are about to request friendship from a user named <b>%s</b>, are you sure you want to do that ? The user may reject your request, if you are sure click <a href="relations.php?do=add&amp;uid=%d&amp;sure=1">here</a>', $cna, $ar1['username'], $uid));
        }
    } else {
        $message = array(
            'subject' => sprintf('Friendship request from %s', $cna) ,
            'msg' => format_comment(sprintf('Hey, [url=userdetails.php?id=%d]%s[/url] would like to be your firend, you can manage your firendship requests [url=relations.php#requests]here[/url]', $cid, $cna)) ,
            'sender' => (int)$INSTALLER09['bot_id'],
            'receiver' => $uid,
            'added' => (int)TIME_NOW
        );
        $relation = array(
