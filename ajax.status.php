...
case 'edit':
    if (!empty($ss)) {
        if (sql_query('UPDATE '.TBL_USTATUS.' SET last_status = '.sqlesc(url2short($ss)).', last_update = '.TIME_NOW.' WHERE userid ='.sqlesc($CURUSER['id']))) $return = jsonmsg(array(
            ...
            'There was an error, mysql error'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)) ,
            false
        ));
    } else $return = jsonmsg(array(
        'nothing to update, string empty',
        false
    ));
    break;
