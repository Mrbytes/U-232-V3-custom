...
<img src='".safe($arr["poster"])."' width='150' height='195' alt='".safe($arr["name"])."' />
<br /><br />
...
<tr><td align='left'>Uploader :&nbsp;<b><a href='userdetails.php?id=".(int)$arr["owner"]."' target='_blank'>".safe($arr["username"])."</a></b>&nbsp;&nbsp;";
...
<tr><td align='left'>Added :&nbsp;<b>".get_date($arr["added"], 'LONG', 0, 1)."</b></td></tr>
        ...
        $HTMLOUT.= end_main_frame();
        echo stdhead("Details for ".safe($arr["name"])."").$HTMLOUT.stdfoot();
    }
} elseif ($mode == "preview") {
    $id = isset($_GET["id"]) ? 0 + $_GET["id"] : 0;
    if ($id == 0) stderr("Err", "Not a valid id");
		...
		<head>
		<title>Preview for - ".safe($arr["name"])."</title>
		</head>
		<body>
...
<td><a href='subtitles.php?mode=details&amp;id=".(int)$arr["id"]."' onmouseover=\"tip('<img src=\'".safe($arr["poster"])."\' width=\'100\'>')\" onmouseout=\"untip()\">".safe($arr["name"])."</a></td>
...
<td align='center'>".safe($arr["hits"])."</td>
...
<a href='subtitles.php?mode=delete&amp;id=".$arr["id"]."'><img src='pic/drop.png' alt='Delete Sub' title='Delete Sub' style='border:none;padding:2px;' /></a>
            ...
            $HTMLOUT.= "<td align='center'><a href='userdetails.php?id=".(int)$arr["owner"]."'>".safe($arr["username"])."</a></td></tr>";
        }
        $HTMLOUT.= "</table>";
    }
    $HTMLOUT.= end_frame();
