...
if ($game) {
    ...
    $sql = sql_query('SELECT uploaded, downloaded, bjwins, bjlosses '.'FROM '.TBL_USERS.' '.'WHERE id = '.sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
    ...
    if ($start_ != 'yes') {
        $playeres = sql_query("SELECT * FROM ".TBL_BLACKJACK." WHERE userid = " . sqlesc($CURUSER['id']));
        ...
        foreach ($arr as $card_id) {
            $used_card = sql_query("SELECT * FROM ".TBL_CARDS." WHERE id=" . sqlesc($card_id));
    ...
    if ($_POST["game"] == 'hit') {
        if ($start_ == 'yes') {
            ...
            if ($INSTALLER09['ratio_free'] === false && $ratio < $required_ratio) stderr("Sorry " . $CURUSER["username"], "Your ratio is lower than the requirement of " . $required_ratio . "%.");
            $res = sql_query("SELECT status, gameover FROM ".TBL_BLACKJACK." WHERE userid = " . sqlesc($CURUSER['id']));
            ...
            foreach ($cardids as $cardid) {
                ...
                $cardres = sql_query("SELECT points, pic FROM ".TBL_CARDS." WHERE id='$cardid'");
            ...
            sql_query("INSERT INTO ".TBL_BLACKJACK." (userid, points, cards, date) VALUES(" . sqlesc($CURUSER['id']) . ", '$points', '" . join(" ", $cardids2) . "', $now)");
            if ($points < 21) {
            ...
            for ($i = 0; $i < $aces; $i++) $points+= ($points < 11 && $aces - $i == 1 ? 11 : 1);
            sql_query("UPDATE ".TBL_BLACKJACK." SET points='$points', cards='" . $cards . " " . $cardid . "' WHERE userid=" . sqlesc($CURUSER['id']));
        }
        if ($points == 21 || $points > 21) {
            $waitres = sql_query("SELECT COUNT(userid) AS c FROM ".TBL_BLACKJACK." WHERE status = 'waiting' AND userid != " . sqlesc($CURUSER['id']));
        ...
        if ($points == 21) {
            if ($waitarr['c'] > 0) {
                $r = sql_query("SELECT bj.*, u.username, u.uploaded, u.downloaded, u.bjwins, u.bjlosses FROM ".TBL_BLACKJACK." AS bj LEFT JOIN ".TBL_USERS." AS u ON u.id=bj.userid WHERE bj.status='waiting' AND bj.userid != " . sqlesc($CURUSER['id']) . " ORDER BY bj.date ASC LIMIT 1");
                $a = mysqli_fetch_assoc($r);
                if ($a["points"] != 21) {
                    $winorlose = "you won " . mksize($mb);
                    sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded + $mb, bjwins = bjwins + 1 WHERE id=" . sqlesc($CURUSER['id']));
                    sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded - $mb, bjlosses = bjlosses + 1 WHERE id=" . sqlesc($a['userid']));
                ...
                sql_query("INSERT INTO ".TBL_MESSAGES." (sender, receiver, added, msg, subject) VALUES(0, " .sqlesc($a['userid']) . ", $now, $msg, $subject)");
                $mc1->delete_value('inbox_new_'.$a['userid']);
                $mc1->delete_value('inbox_new_sb_'.$a['userid']);
                sql_query("DELETE FROM ".TBL_BLACKJACK." WHERE userid IN (" . sqlesc($CURUSER['id']) . ", " . sqlesc($a['userid']) . ")");
                ...
                sql_query("UPDATE ".TBL_BLACKJACK." SET status = 'waiting', date=" . $now . ", gameover = 'yes' WHERE userid = " . sqlesc($CURUSER['id']));
            ...
            if ($waitarr['c'] > 0) {
                ...
                if ($a["points"] > 21) {
                    ...
                    sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded + $mb, bjwins = bjwins + 1 WHERE id=" . sqlesc($a['userid']));
                    sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded - $mb, bjlosses = bjlosses + 1 WHERE id=" . sqlesc($CURUSER['id']));
                ...
                sql_query("INSERT INTO ".TBL_MESSAGES." (sender, receiver, added, msg, subject) VALUES(0, " . $a['userid'] . ", $now, $msg, $subject)");
                $mc1->delete_value('inbox_new_'.$a['userid']);
                $mc1->delete_value('inbox_new_sb_'.$a['userid']);
                sql_query("DELETE FROM ".TBL_BLACKJACK." WHERE userid IN (" . sqlesc($CURUSER['id']) . ", " . sqlesc($a['userid']) . ")");
                $HTMLOUT.= "<tr><td align='center'>Your opponent was " . htmlsafechars($a["username"]) . ", he/she had " . htmlsafechars($a['points']) . " points, $winorlose.<br /><br /><b><a href='blackjack.php'>Play again</a></b></td></tr>";
      ...
      <td align='center'><form method='post' action='" . $_SERVER['PHP_SELF'] . "'><input type='hidden' name='game' value='stop' readonly='readonly' /><input type='submit' value='Stay' /></form></td>
        ...
        if ($waitarr['c'] > 0) {
            ...
            if ($a["points"] == $playerarr['points']) {
                ...
                sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded " . $st_query . " 1 WHERE id=" . sqlesc($CURUSER['id']));
                sql_query("UPDATE ".TBL_USERS." SET uploaded = uploaded " . $nd_query . " 1 WHERE id=" . sqlesc($a['userid']));
                $update['uploaded'] = ($a['uploaded'] + $mb);
                $update['uploaded_loser'] = ($User['uploaded'] - $mb);
                $update['bjwins'] = ($a['bjwins'] + 1);
                $update['bjlosses'] = ($User['bjlosses'] + 1);
    ...
    $sql = sql_query('SELECT bjwins, bjlosses '.'FROM '.TBL_USERS.' '.'WHERE id = '.sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
    $User = mysqli_fetch_assoc($sql);
    $User['bjwins'] = (int)$User['bjwins'];
    $User['bjlosses'] = (int)$User['bjlosses'];
