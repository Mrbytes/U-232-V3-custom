...
elseif ($action == "security") {
    ...
    if (!mkglobal("email:chpassword:passagain:chmailpass:secretanswer")) stderr("Error", $lang['takeeditcp_no_data']);
    if ($chpassword != "") {
        if (strlen($chpassword) > 40) stderr("Error", $lang['takeeditcp_pass_long']);
        if ($chpassword != $passagain) stderr("Error", $lang['takeeditcp_pass_not_match']);
        $secret = mksecret();
    ...
    if ($secretanswer != '') {
        if (strlen($secretanswer) > 40) stderr("Sorry", "secret answer is too long (max is 40 chars)");
        if (strlen($secretanswer) < 6) stderr("Sorry", "secret answer is too sort (min is 6 chars)");
...
elseif ($action == "personal") {
    ...
    if (isset($_POST["title"]) && $CURUSER["class"] >= UC_VIP && ($title = $_POST["title"]) != $CURUSER["title"]) {
        ...
        if (in_array(strtolower($title) , ($notallow))) stderr("Error", "Invalid custom title!");
    ...
    if ($CURUSER['birthday'] == '0') {
        ...
        if ($year == '0000') stderr("Error", "Please set your birth year.");
        if ($month == '00') stderr("Error", "Please set your birth month.");
        if ($day == '00') stderr("Error", "Please set your birth day.");
        if (!checkdate($month, $day, $year)) stderr("Error", "<br /><div id='error' align='center'><font color='red' size='+1'>The date entered is not a valid date, please try again</font></div><br />");
