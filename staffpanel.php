...
if (!isset($staff_classes[$CURUSER['class']]) || !$CURUSER) stderr('Error', 'Access Denied!');
/**
 ...
 *
 */
if ($INSTALLER09['staffpanel_online'] == 0) stderr('Information', 'The staffpanel is currently offline for maintainance work');
...
if (in_array($tool, $staff_tools) and file_exists(ADMIN_DIR.$staff_tools[$tool].'.php')) {
    ...
    if ($action == 'delete' && is_valid_id($id) && $staff_classes[$CURUSER['class']]['delete']) {
        ...
        if (mysqli_affected_rows($GLOBALS["___mysqli_ston"])) {
            ...
            exit();
        } else stderr('Error', 'There was a database error, please retry.');
        ...
        if ($action == 'edit' && $CURUSER['class'] < $av_class) stderr('Error', 'You are not allowed to edit this page.');
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $errors = array();
            if (empty($page_name)) $errors[] = 'The page name cannot be empty.';
            if (empty($file_name)) $errors[] = 'The filename cannot be empty.';
            if (empty($description)) $errors[] = 'The description cannot be empty.';
            if (!isset($staff_classes[$av_class])) $errors[] = 'The selected class is not a valid staff class.';
            if (!is_file($file_name.'.php') && !empty($file_name) && !preg_match('/.php/', $file_name)) $errors[] = 'Inexistent php file.';
            if (strlen($page_name) < 4 && !empty($page_name)) $errors[] = 'The page name is too short (min 4 chars).';
            if (strlen($page_name) > 80) $errors[] = 'The page name is too long (max 30 chars).';
            if (strlen($file_name) > 80) $errors[] = 'The filename is too long (max 30 chars).';
            ...
            if (empty($errors)) {
                if ($action == 'add') {
                    ...
                    if (!$res) {
                        if (((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false)) == 1062) $errors[] = "This filename is already submited.";
    ...
    <select name='av_class'>";
        foreach ($staff_classes as $class => $value) {
            if ($CURUSER['class'] < $class) continue;
     ...
     <form method='post' action='{$_SERVER['PHP_SELF']}'><input type='submit' value='Cancel' /></form>
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            foreach ($mysql_data as $key => $value) $db_classes[$value['av_class']][] = $value['av_class'];
            $i = 1;
