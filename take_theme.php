...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($sid > 0 && $sid != $CURUSER['id']) sql_query('UPDATE '.TBL_USERS.' SET stylesheet='.sqlesc($sid).' WHERE id = '.sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
...
$ss_r = sql_query("SELECT id, name from ".TBL_STYLESHEETS." ORDER BY id ASC") or sqlerr(__FILE__, __LINE__);
