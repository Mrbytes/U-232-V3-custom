...
if (!isset($CURUSER)) stderr("Error", "{$lang['rate_login']}");
if (!mkglobal("rating:id")) stderr("Error", "{$lang['rate_miss_form_data']}");
$id = 0 + $id;
if (!$id) stderr("Error", "{$lang['rate_invalid_id']}");
$rating = 0 + $rating;
if ($rating <= 0 || $rating > 5) stderr("Error", "{$lang['rate_invalid']}");
$f_r = sql_query("SELECT owner, numratings, ratingsum , IF(numratings < {$INSTALLER09['minvotes']}, NULL, ROUND(ratingsum / numratings, 1)) AS rating FROM ".TBL_TORRENTS." WHERE id = $id");
$r_f = mysqli_fetch_assoc($f_r);
if (!$r_f) stderr("Error", "{$lang['rate_torrent_not_found']}");
$time_now = TIME_NOW;
$res = sql_query("INSERT INTO ".TBL_RATING." (torrent, user, rating, added) VALUES ($id, ".$CURUSER["id"].", $rating, $time_now)");
if (!$res) {
...
sql_query("UPDATE ".TBL_TORRENTS." SET numratings = numratings + 1, ratingsum = ratingsum + $rating WHERE id = $id");
