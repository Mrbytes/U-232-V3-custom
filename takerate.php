...
$f_r = sql_query("SELECT owner, numratings, ratingsum , IF(numratings < {$INSTALLER09['minvotes']}, NULL, ROUND(ratingsum / numratings, 1)) AS rating FROM ".TBL_TORRENTS." WHERE id = $id") or sqlerr(__FILE__, __LINE__);
$r_f = mysqli_fetch_assoc($f_r);
if (!$r_f) stderr("Error", "{$lang['rate_torrent_not_found']}");
$time_now = TIME_NOW;
$res = sql_query("INSERT INTO ".TBL_RATING." (torrent, user, rating, added) VALUES (".sqlesc($id).", ".sqlesc($CURUSER["id"]).", ".sqlesc($rating).", ".sqlesc($time_now).")");
if (!$res) {
