...
if (mysqli_num_rows($res) == 0) {
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        $color = '#FF4763';
        if (strpos($arr['txt'], 'Warned')) $color = "#FF0000";
        if (strpos($arr['txt'], 'Leech warned')) $color = "#9ED9D0";
        if (strpos($arr['txt'], 'Download possible')) $color = "#62D962";
        if (strpos($arr['txt'], 'Immunity enabled')) $color = "#FFFF00";
        if (strpos($arr['txt'], 'Enabled')) $color = "#47FFE3";
        if (strpos($arr['txt'], 'Donor')) $color = "#FF8112";
        if (strpos($arr['txt'], 'Paranoia level changed')) $color = "#E8001F";
        if (strpos($arr['txt'], 'Upload Total')) $color = "#14ED00";
        if (strpos($arr['txt'], 'Download total')) $color = "#5A63C7";
        if (strpos($arr['txt'], 'Invite total')) $color = "#54ACBA";
        if (strpos($arr['txt'], 'Seedbonus total')) $color = "#BA6154";
        if (strpos($arr['txt'], 'Reputuation total')) $color = "#57AD00";
        if (strpos($arr['txt'], 'Promoted')) $color = "#E01E00";
        if (strpos($arr['txt'], 'Demoted')) $color = "#BA5480";
        if (strpos($arr['txt'], 'website')) $color = "#00CFA2";
