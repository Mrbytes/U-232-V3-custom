...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "deluser" && (!empty($_POST["userid"]))) {
        sql_query("DELETE FROM ".TBL_USERS." WHERE id IN (".implode(", ", $_POST['userid']).") ");
    ...
    if ($action == "disable" && (!empty($_POST["userid"]))) {
        sql_query("UPDATE ".TBL_USERS." SET enabled='no' WHERE id IN (".implode(", ", $_POST['userid']).") ");
    ...
    if ($action == "mail" && (!empty($_POST["userid"]))) {
        $res = sql_query("SELECT id, email, modcomment, username, added, last_access FROM ".TBL_USERS." WHERE id IN (".implode(", ", $_POST['userid']).") ORDER BY last_access DESC ");
        ...
        if ($record_mail) {
            ...
            if ($count > 0 && $mail) sql_query("UPDATE ".TBL_AVPS." SET value_i='$date', value_u='$count', value_s='$userid' WHERE arg='inactivemail'") or sqlerr(__FILE__, __LINE__);
        }
...
$res = sql_query("SELECT COUNT(id) FROM ".TBL_USERS." WHERE last_access<$dt AND status='confirmed' AND enabled='yes' ORDER BY last_access DESC");
$row = mysqli_fetch_array($res);
$count = $row[0];
$perpage = 15;
$pager = pager($perpage, $count, "staffpanel.php?tool=inactive&amp;");
$res = sql_query("SELECT id,username,class,email,uploaded,downloaded,last_access FROM ".TBL_USERS." WHERE last_access<$dt AND status='confirmed' AND enabled='yes' ORDER BY last_access DESC {$pager['limit']}") or sqlerr(__FILE__, __LINE__);
...
if ($count_inactive > 0) {
    ...
    if ($record_mail) {
        $ress = sql_query("SELECT ".TBL_AVPS.".value_s AS userid, ".TBL_AVPS.".value_i AS last_mail, ".TBL_AVPS.".value_u AS mails, ".TBL_USERS.".username FROM ".TBL_AVPS." LEFT JOIN ".TBL_USERS." ON ".TBL_AVPS.".value_s= ".TBL_USERS.".id WHERE ".TBL_AVPS.".arg='inactivemail' LIMIT 1");
