...
echo $lang['usersearch_class'] ?></td>
  <td <?php
echo (isset($_POST['c']) && !empty($_POST['c'])) ? $highlight : "" ?>><select name="c"><option value=''>(any)</option>
  <?php
...
if (count($_POST) > 0); //&& isset($_POST['n']))
{
    ...
    if (mysqli_num_rows($res) == 0) stdmsg($lang['usersearch_warn'], $lang['usersearch_nouser']);
        ...
        if ($count > $perpage) echo $pager['pagertop'];
      ...
      WHERE p.user_id = ".$user['id']." AND f.min_class_read <= ".$CURUSER['class']) or sqlerr(__FILE__, __LINE__);
            ...
            $n_comments = $n[0];
            $ids.= $user['id'].':';
            echo "<tr><td><b><a href='userdetails.php?id=".$user['id']."'>".$user['username']."</a></b>".($user["donor"] == "yes" ? "<img src='pic/star.gif' alt=\"Donor\" />" : "").($user["warned"] == "yes" ? "<img src=\"pic/warned.gif\" alt=\"Warned\" />" : "")."</td>
        ...
        echo "</table>";
        if ($count > $perpage) echo $pager['pagerbottom'];
?>
<br />
...
if (isset($pagemenu)) echo ("<p>$pagemenu<br />$browsemenu</p>");
echo stdfoot();
die;
?>
