...
if (isset($_GET['remove'])) {
    if ($CURUSER['class'] < UC_STAFF) stderr('Error', 'Thou infectious onion-eyed haggard! Only Admin and up can remove members from the list!');
    ...
    if (mysqli_affected_rows($GLOBALS["___mysqli_ston"]) == 0) stderr('Error', 'No one was deleted!');
    else write_log('<b>'.$CURUSER['username'].'</b> Removed:<br />'.$removed_log.' <br />from watched users');
