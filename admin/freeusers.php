...
if ($remove) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            $modcomment = sqlesc(get_date(TIME_NOW, 'DATE', 1)." - Freeleech On All Torrents removed by ".$CURUSER['username']." \n");
            $msgs_buffer[] = '(0,'.$arr['id'].','.TIME_NOW.', '.sqlesc($msg).', \'Freeleech Notice!\')';
...
if ($count == 0) $HTMLOUT.= '<p align="center"><b>Nothing here</b></p>';
    ...
    while ($arr2 = mysqli_fetch_assoc($res2)) {
        ...
        if ($arr2['class'] > UC_ADMINISTRATOR && $arr2['id'] != $CURUSER['id']) $HTMLOUT.= "</td><td align='left'>Until ".get_date($arr2['free_switch'], 'DATE')." 
(".mkprettytime($arr2['free_switch'] - TIME_NOW)." to go)"."</td><td align='left'><font color='red'>Not Allowed</font></td>
</tr>";
        else $HTMLOUT.= "</td><td align='left'>Until ".get_date($arr2['free_switch'], 'DATE')." 
