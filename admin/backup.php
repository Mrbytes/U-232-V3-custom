...
if (is_array($required_class)) {
    if (!in_array($CURUSER['class'], $required_class)) stderr("Error", "Access denied!");
} else {
    if ($CURUSER['class'] <> $required_class) stderr("Error", "Access denied!");
}
...
if (empty($mode)) {
 ...
 Click<a href='{$INSTALLER09['baseurl']}/staffpanel.php?tool=backup&amp;mode=check'><b>&nbsp;Here&nbsp;</b></a>to check config.</h1>";
    ...
    if (isset($_GET['backedup'])) $HTMLOUT.= stdmsg("Success", "Database backed up.");
    else if (isset($_GET['deleted'])) $HTMLOUT.= stdmsg("Success", "Backup(s) Deleted.");
    else if (isset($_GET['noselection'])) $HTMLOUT.= stdmsg("Error", "Please select a backup to delete.");
    ...
    if ($write2log) write_log($CURUSER['username'].'('.get_user_class_name($CURUSER['class']).') successfully backed-up the database.');
    ...
    //print $filename;
    //exit();
    if (!is_file($filename)) stderr('Error', 'Inexistent filename.');
...
} else stderr('Sorry', 'Unknown action!');
?>
