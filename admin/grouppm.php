...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($subject)) $err[] = "Your message doesn't have a subject";
    if (empty($msg)) $err[] = "There is not any text in your message!";
    //$msg .= "\n This is a group message !";
    if (empty($groups)) $err[] = "You have to select a group to send your message";
    if (sizeof($err) == 0) {
...
$groups = array(
    ...
    array(
        "opname" => "Related to you",
        "minclass" => UC_USER,
        "ops" => array(
            array(
                "all_friends" => "Your friends"
            )
        )
    )
);
