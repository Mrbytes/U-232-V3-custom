...
PS. Your donator status will last for an extra ".$dur." on top of your current donation status, and can be found on your user details page. It can only be seen by you.");
    ...
    } else $user_class = get_user_class_name($user['class'])." [ no change ]";
    //=== change title if blank
    if ($user['title'] == '') $updateset[] = "title = 'ChangeMe'"; //=== change this lol
...
if ($class == '-' || !is_valid_id($class)) if ($search != '' || $class) {
...
for ($i = 97; $i < 123; ++$i) {
    $l = chr($i);
    $L = chr($i - 32);
    if ($l == $letter) $HTMLOUT.= "<b>$L</b>\n";
    else $HTMLOUT.= "<a href='staffpanel.php?tool=paypal_manual_confirm&amp;searchl=1&amp;letter=$l'><b>$L</b></a>\n";
}
if (isset($_GET['search']) || isset($_GET['searchl'])) {
    ...
    for ($i = 1; $i <= $pages; ++$i) if ($i == $page) $pagemenu.= "<b>$i</b>\n";
    else $pagemenu.= "<a href='?tool=paypal_manual_confirm&searchl=1&amp;$q1&amp;page=$i'><b>$i</b></a>\n";
    if ($page == 1) $browsemenu.= "<b>&lt;&lt; Prev</b>";
    else $browsemenu.= "<a href='?tool=paypal_manual_confirm&searchl=1&amp;$q1&amp;page=".($page - 1)."'><b>&lt;&lt; Prev</b></a>";
    $browsemenu.= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
...
<td align='left' class='$class'>".get_user_class_name($arr["class"])."</td></tr>\n";
    ...
    if (isset($_GET['search']) && $num || isset($_GET['searchl']) && $num) $HTMLOUT.= "</table>\n";
    else $HTMLOUT.= "<h1>nothing found!</h1>";
    if (isset($_GET['search']) && $num || isset($_GET['searchl']) && $num) $HTMLOUT.= "<p>$pagemenu<br />$browsemenu</p>";
}
//=== end search users
//======= regular page start
