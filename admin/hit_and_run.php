...
$query = (isset($_GET['really_bad']) ? 'SELECT COUNT(*) FROM '.TBL_SNATCHED.' LEFT JOIN '.TBL_USERS.' ON '.TBL_USERS.'.id = '.TBL_SNATCHED.'.userid WHERE '.TBL_SNATCHED.'.finished = \'yes\' AND '.TBL_SNATCHED.'.hit_and_run > 0 AND '.TBL_USERS.'.hit_and_run_total > 2' : 'SELECT COUNT(*) FROM '.TBL_SNATCHED.' WHERE `finished` = \'yes\' AND `hit_and_run` > 0');
$HTMLOUT = '';
...
$HTMLOUT.= '<h2>'.(!isset($_GET['really_bad']) ? 'Current Hit and Runs who still have a chance' : 'Hit and Runs with no chance').'</h2><br /> 
		...
		<td class="colhead">Actions</td>' : '<tr><td> no hit and runners at the moment...</td>').'</tr>';
