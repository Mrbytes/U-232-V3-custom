...
if (isset($_GET["action1"]) && htmlsafechars($_GET["action1"]) == "list") {
    $res2 = sql_query("SELECT userid, seeder, torrent, agent FROM ".TBL_PEERS." WHERE connectable='no' ORDER BY userid DESC") or sqlerr(__FILE__, __LINE__);
    ...
    $result = sql_query("select distinct userid from ".TBL_PEERS." where connectable = 'no'");
    ...
    if (mysqli_num_rows($res2) == 0) $HTMLOUT.= "<p align='center'><b>{$lang['non_con_all']}</b></p>\n";
        ...
        while ($arr2 = mysqli_fetch_assoc($res2)) {
            $r2 = sql_query("SELECT username FROM ".TBL_USERS." WHERE id=".sqlesc($arr2['userid'])) or sqlerr(__FILE__, __LINE__);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = sql_query("SELECT distinct userid FROM ".TBL_PEERS." WHERE connectable='no'");
    while ($dat = mysqli_fetch_assoc($query)) {
        $subject = "Connectability";
        sql_query("INSERT INTO ".TBL_MESSAGES." (sender, receiver, added, msg, subject) VALUES (0, ".sqlesc($dat['userid']).", '".TIME_NOW."', ".sqlesc($msg).", ".sqlesc($subject).")") or sqlerr(__FILE__, __LINE__);
    }
    sql_query("INSERT INTO ".TBL_NOTCONNECTABLEPMLOG." (user, date) VALUES (".sqlesc($CURUSER['id']).", $dt)") or sqlerr(__FILE__, __LINE__);
...
<form method='post' action='staffpanel.php?tool=findnotconnectable&amp;action=findnotconnectable'>";
    if (isset($_GET["returnto"]) || isset($_SERVER["HTTP_REFERER"])) {
        $HTMLOUT.= "<input type='hidden' name='returnto' value='".(isset($_GET["returnto"]) ? htmlsafechars($_GET["returnto"]) : $_SERVER["HTTP_REFERER"])."' />";
    }
    $receiver = '';
    // default message
...
if (isset($_GET["action1"]) == "") {
    $getlog = sql_query("SELECT * FROM `".TBL_NOTCONNECTABLEPMLOG."` ORDER BY date DESC LIMIT 20");
    ...
    while ($arr2 = mysqli_fetch_assoc($getlog)) {
        $r2 = sql_query("SELECT username FROM ".TBL_USERS." WHERE id=".sqlesc($arr2['user'])) or sqlerr(__FILE__, __LINE__);
        $a2 = mysqli_fetch_assoc($r2);
