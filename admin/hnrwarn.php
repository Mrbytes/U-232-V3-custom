...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($act == "disable") {
        ...
        if (count($pms)) {
            ...
            } else stderr("Err", "Something went wrong! Q1 - ".$q_err."<br />Q2 - ".$q1_err);
        }
    }
    exit;
}
...
if ($count == 0) $HTMLOUT.= stdmsg("hey", "There is no ".strtolower($title));
    ...
    while ($a = mysqli_fetch_assoc($g)) {
        $tip = ($do == "hnrwarn" ? "Hit and run Warned for : ".htmlsafechars($a["warn_reason"])."<br />" : "Disabled for ".htmlsafechars($a["disable_reason"]));
        $HTMLOUT.= "<tr>
