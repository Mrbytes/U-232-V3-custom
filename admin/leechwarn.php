...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_uids == 0 || count($_uids) == 0) stderr("Err", "Looks like you didn't select any user !");
    $valid = array(
        "unwarn",
        "disable",
        "delete"
    ...
    if ($act == "disable") {
        ...
        } else stderr("Err", "Something went wrong 3!");
    } elseif ($act == "unwarn") {
...
case "leechwarn":
    $query = "SELECT id, username, class, downloaded, uploaded, IF(downloaded>0, round((uploaded/downloaded),2), '---') as ratio, warn_reason, leechwarn, added, last_access FROM ".TBL_USERS." WHERE leechwarn>='1' ORDER BY last_access DESC, leechwarn DESC ";
...
if ($count == 0) $HTMLOUT.= stdmsg("hey", "There is no ".strtolower($title));
else {
