...
$sql = "SELECT p.id, p.userid, p.torrent, p.passkey, p.peer_id, p.ip, p.port, p.uploaded, p.downloaded, p.to_go, p.seeder, p.started, p.last_action, p.connectable, p.agent, p.finishedat, p.downloadoffset, p.uploadoffset, u.username, t.name "."FROM ".TBL_PEERS." AS p "."LEFT JOIN ".TBL_USERS." AS u ON u.id=p.userid "."LEFT JOIN ".TBL_TORRENTS." AS t ON t.id=p.torrent WHERE started != '0'"."ORDER BY p.started DESC {$pager['limit']}";
