...
if (!empty($pkey)) {
    $q0 = sql_query('SELECT * FROM '.TBL_USERS.' where passkey = '.sqlesc($pkey)) or sqlerr(__FILE__, __LINE__);
...
$res = sql_query('SELECT name, owner, vip, category, filename FROM '.TBL_TORRENTS.' WHERE id = '.sqlesc($id)) or sqlerr(__FILE__, __LINE__);
...
if (happyHour('check') && happyCheck('checkid', $row['category'])) {
    ...
    sql_query('INSERT INTO '.TBL_HAPPYHOUR.' (userid, torrentid, multiplier ) VALUES ('.sqlesc($CURUSER['id']).','.sqlesc($id).','.sqlesc($multiplier).')') or sqlerr(__FILE__, __LINE__);
...
sql_query("UPDATE ".TBL_TORRENTS." SET hits = hits + 1 WHERE id = ".sqlesc($id));
/** free mod by pdq **/
/** freeslots/doubleseed by pdq **/
if (isset($_GET['slot'])) {
    $added = (TIME_NOW + 14 * 86400);
    $slots_sql = sql_query('SELECT * FROM '.TBL_FREESLOTS.' WHERE torrentid = '.sqlesc($id).' AND userid = '.sqlesc($CURUSER['id']));
    ...
    if ($_GET['slot'] == 'free') {
        ...
        sql_query('UPDATE '.TBL_USERS.' SET freeslots = freeslots - 1 WHERE id = '.sqlesc($CURUSER['id']).' LIMIT 1') or sqlerr(__FILE__, __LINE__);
        if ($used_slot && $slot['doubleup'] == 'yes') sql_query('UPDATE '.TBL_FREESLOTS.' SET free = "yes", addedfree = '.$added.' WHERE torrentid = '.$id.' AND userid = '.$CURUSER['id'].' AND doubleup = "yes"') or sqlerr(__FILE__, __LINE__);
        elseif ($used_slot && $slot['doubleup'] == 'no') sql_query('INSERT INTO '.TBL_FREESLOTS.' (torrentid, userid, free, addedfree) VALUES ('.sqlesc($id).', '.sqlesc($CURUSER['id']).', "yes", '.$added.')') or sqlerr(__FILE__, __LINE__);
        else sql_query('INSERT INTO '.TBL_FREESLOTS.' (torrentid, userid, free, addedfree) VALUES ('.sqlesc($id).', '.sqlesc($CURUSER['id']).', "yes", '.$added.')') or sqlerr(__FILE__, __LINE__);
    }
    /** doubleslot **/
    elseif ($_GET['slot'] == 'double') {
        ...
        $CURUSER['freeslots'] = ($CURUSER['freeslots'] - 1);
        sql_query('UPDATE '.TBL_USERS.' SET freeslots = freeslots - 1 WHERE id = '.sqlesc($CURUSER['id']).' LIMIT 1') or sqlerr(__FILE__, __LINE__);
        if ($used_slot && $slot['free'] == 'yes') sql_query('UPDATE '.TBL_FREESLOTS.' SET doubleup = "yes", addedup = '.$added.' WHERE torrentid = '.sqlesc($id).' AND userid = '.sqlesc($CURUSER['id']).' AND free = "yes"') or sqlerr(__FILE__, __LINE__);
        elseif ($used_slot && $slot['free'] == 'no') sql_query('INSERT INTO '.TBL_FREESLOTS.' (torrentid, userid, doubleup, addedup) VALUES ('.sqlesc($id).', '.sqlesc($CURUSER['id']).', "yes", '.$added.')') or sqlerr(__FILE__, __LINE__);
        else sql_query('INSERT INTO '.TBL_FREESLOTS.' (torrentid, userid, doubleup, addedup) VALUES ('.sqlesc($id).', '.sqlesc($CURUSER['id']).', "yes", '.$added.')') or sqlerr(__FILE__, __LINE__);
...
if (!isset($CURUSER['passkey']) || strlen($CURUSER['passkey']) != 32) {
    ...
    sql_query('UPDATE '.TBL_USERS.' SET passkey='.sqlesc($CURUSER['passkey']).' WHERE id='.$CURUSER['id']);
    $mc1->begin_transaction('MyUser_'.$CURUSER['id']);
    $mc1->update_row(false, array(
