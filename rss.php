...
if (!empty($passkey)) {
    if (strlen($passkey) != 32) die("Your passkey is not long enough! Go to ".$INSTALLER09['site_name']." and reset your passkey");
        ...
        if (get_row_count(TBL_USERS, "where passkey=".sqlesc($passkey)) != 1) die("Your passkey is invalid !Go to ".$INSTALLER09['site_name']." and reset your passkey");
    }
...
$res = sql_query("SELECT t.id,t.name,t.descr,t.size,t.category,t.seeders,t.leechers,t.added, c.name as catname FROM ".TBL_TORRENTS." as t LEFT JOIN ".TBL_CATEGORIES." as c ON t.category = c.id WHERE $where t.visible='yes' ORDER BY t.added DESC LIMIT 15") or sqlerr(__FILE__, __LINE__);
