...
if (!isset($Name)) stderr($lang['userimages_hmm'], "{$lang['userimages_nouser']}");
...
if ($CURUSER['username'] != $Name) {
    $staffnames = array(
        ...
        'Tam',
        'System'
    ); // :P paranoid users here
    if (in_array($Name, $staffnames)) stderr($lang['userimages_forbidden'], "{$lang['userimages_shoo']}");
}
...
if (isset($_GET["delete"]) && ($CURUSER['class'] >= UC_STAFF)) {
    ...
    if ($delhash != $_GET['delhash']) stderr($lang['userimages_umm'], "{$lang['userimages_waydh']}");
    ...
    else stderr($lang['userimages_hey'], "{$lang['userimages_imagenf']}");
    if (isset($_GET["type"]) && $_GET["type"] == 2) header("Refresh: 2; url={$INSTALLER09['baseurl']}/userimages.php?images=2&user=$_SESSION[picname]");
    else header("Refresh: 2; url={$INSTALLER09['baseurl']}/userimages.php?images=1&user=$_SESSION[picname]");
...
<br />";
        } else $HTMLOUT.= "{$lang['userimages_noimage']}";
    }
}
