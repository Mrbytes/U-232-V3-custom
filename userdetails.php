...
if ($CURUSER['id'] == $user['id']) $HTMLOUT.= "<h1><a href='{$INSTALLER09['baseurl']}/usercp.php?action=default'>Edit My Profile</a></h1>
              <h1><a href='{$INSTALLER09['baseurl']}/view_announce_history.php'>View My Announcements</a></h1>";
if ($CURUSER['id'] != $user['id']) $HTMLOUT.= "<h1><a href='{$INSTALLER09['baseurl']}/sharemarks.php?id=$id'>View sharemarks</a></h1>\n";
...
if (($CURUSER['id'] !== $user['id']) && ($CURUSER['class'] >= UC_STAFF)) {
    ...
    $HTMLOUT.= '<tr><td class="rowhead">Watched User</td>
                            <td align="left">'.($user['watched_user'] > 0 ? 'Currently being watched since  '.get_date($user['watched_user'], '').' ' : ' Not currently being watched ').$the_flip_box.'
    ...
    if (!empty($user_stats['modcomment'])) $HTMLOUT.= "<tr><td class='rowhead'>System Comments</td><td align='left'>".($user_stats['modcomment'] != '' ? $the_flip_box_7.'<div align="left" id="box_7" style="display:none"><hr />'.format_comment($user_stats['modcomment']).'</div>' : '')."</td></tr>\n";
}
//==Begin blocks
...
if ($CURUSER['class'] >= UC_STAFF && $user["class"] < $CURUSER['class']) {
    ...
    if ($CURUSER["class"] == UC_MAX) {
        ...
        if ($donor) {
            ...
            if ($donoruntil == '0') $HTMLOUT.= "Arbitrary duration";
            else {
                $HTMLOUT.= "<b>".$lang['userdetails_donor2']."</b> ".get_date($user['donoruntil'], 'DATE')." ";
                $HTMLOUT.= " [ ".mkprettytime($donoruntil - TIME_NOW)." ] To go\n";
            }
        } else {
            $HTMLOUT.= "{$lang['userdetails_dfor']}<select name='donorlength'><option value='0'>------</option><option value='4'>1 month</option>"."<option value='6'>6 weeks</option><option value='8'>2 months</option><option value='10'>10 weeks</option>"."<option value='12'>3 months</option><option value='255'>Unlimited</option></select>\n";
        }
        ...
        if ($donor) {
            $HTMLOUT.= "<br /><b>{$lang['userdetails_adonor']}</b> <select name='donorlengthadd'><option value='0'>------</option><option value='4'>1 month</option>"."<option value='6'>6 weeks</option><option value='8'>2 months</option><option value='10'>10 weeks</option>"."<option value='12'>3 months</option><option value='255'>Unlimited</option></select>\n";
    ...
    if ($CURUSER['class'] >= UC_STAFF) $HTMLOUT.= "<tr><td class='rowhead'>Freeleech Slots:</td><td colspan='2' align='left'>
    ...
    if ($CURUSER['class'] >= UC_ADMINISTRATOR) {
        ...
        $HTMLOUT.= "<tr><td class='rowhead'".(!$free_switch ? ' rowspan="2"' : '').">Freeleech Status</td>
                <td align='left' width='20%'>".($free_switch ? "<input name='free_switch' value='42' type='radio' />Remove Freeleech Status" : "No Freeleech Status Set")."</td>\n";
        if ($free_switch) {
            if ($user['free_switch'] == 1) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['free_switch'], 'DATE')." (".mkprettytime($user['free_switch'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($downloadpos) {
            if ($user['downloadpos'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['downloadpos'], 'DATE')." (".mkprettytime($user['downloadpos'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($uploadpos) {
            if ($user['uploadpos'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['uploadpos'], 'DATE')." (".mkprettytime($user['uploadpos'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($sendpmpos) {
            if ($user['sendpmpos'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['sendpmpos'], 'DATE')." (".mkprettytime($user['sendpmpos'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($chatpost) {
            if ($user['chatpost'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['chatpost'], 'DATE')." (".mkprettytime($user['chatpost'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($avatarpos) {
            if ($user['avatarpos'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['avatarpos'], 'DATE')." (".mkprettytime($user['avatarpos'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($immunity) {
            if ($user['immunity'] == 1) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['immunity'], 'DATE')." (".mkprettytime($user['immunity'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($leechwarn) {
            if ($user['leechwarn'] == 1) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['leechwarn'], 'DATE')." (".mkprettytime($user['leechwarn'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($warned) {
            if ($user['warned'] == 1) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['warned'], 'DATE')." (".mkprettytime($user['warned'] - TIME_NOW)." to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($game_access) {
            if ($user['game_access'] == 0) $HTMLOUT.= '<td align="center">(Unlimited Duration)</td></tr>';
            else $HTMLOUT.= "<td align='center'>Until ".get_date($user['game_access'], 'DATE')." (".mkprettytime($user['game_access'] - TIME_NOW)." to go)</td></tr>";
        } else {
