...
$HTMLOUT = '';
if (!mkglobal("torrent")) stderr("Error", "missing form data");
$userid = (int)$CURUSER['id'];
if (!is_valid_id($userid)) stderr("Error", "Invalid ID.");
if ($userid != $CURUSER["id"]) stderr("Error", "Access denied.");
$torrentid = 0 + $_GET["torrent"];
if (!is_valid_id($torrentid)) die();
if (!isset($torrentid)) stderr("Error", "Failed. No torrent selected");
...
if ($action == 'delete') {
    ...
    if (!$sure) stderr("Share Bookmark", "Do you really want to mark this bookmark public? Click\n"."<a href='?torrent=$torrentid&amp;action=public&amp;sure=1&amp;h=$hash'>here</a> if you are sure.", FALSE);
    if ($_GET['h'] != $hash) stderr('Error', 'what are you doing?');
    ...
    if (!$sure) stderr("Make Bookmark Private", "Do you really want to mark this bookmark private? Click\n"."<a href='?torrent=$torrentid&amp;action=private&amp;sure=1&amp;h=$hash'>here</a> if you are sure.", FALSE);
    if ($_GET['h'] != $hash) stderr('Error', 'what are you doing?');
    if (!is_valid_id($torrentid)) stderr("Error", "Invalid ID.");
...
if (isset($_POST["returnto"])) $ret = "<a href=\"".htmlsafechars($_POST["returnto"])."\">Go back to whence you came</a>";
else $ret = "<a href=\"bookmarks.php\">Go to My Bookmarks</a><br /><br />
