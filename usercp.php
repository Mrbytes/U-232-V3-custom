...
if ($action == "avatar") {
    ...
    $HTMLOUT.= tr('View avatars', '<input type="radio" name="avatars" '.($CURUSER['avatars'] == 'yes' ? 'checked="checked"' : '').' value="yes" /> Yes
...
elseif ($action == "personal") {
    ...
    //== Default Pms
    if ($action == "default") $HTMLOUT.= begin_table(true);
...
if (!empty($CURUSER['avatar']) && $CURUSER['av_w'] > 5 && $CURUSER['av_h'] > 5) $HTMLOUT.= "<tr><td><img src='{$CURUSER['avatar']}' width='{$CURUSER['av_w']}' height='{$CURUSER['av_h']}' alt='' />
...
else $HTMLOUT.= "<tr><td><img src='{$INSTALLER09['pic_base_url']}forumicons/default_avatar.gif' alt='' /><a href='mytorrents.php'>{$lang['usercp_edit_torrents']}</a><br />
    <a href='friends.php'>{$lang['usercp_edit_friends']}</a><br />
